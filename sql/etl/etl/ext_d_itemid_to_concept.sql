...
CREATE OR REPLACE TABLE d_items_to_concept AS
WITH
counts AS
(
...
CREATE OR REPLACE TABLE d_labitems_to_concept AS
WITH
counts AS
(
    SELECT
...
FROM
    lk_meas_d_labitems_concept src
LEFT JOIN
    counts 
        USING (itemid)
ORDER BY
...
CREATE OR REPLACE TABLE d_micro_to_concept AS
WITH
counts AS
(
    SELECT
